...
En el entorno empresarial actual, las Tecnologías de la Información (TI) constituyen el núcleo estratégico para que las organizaciones modernas operen y mantengan su competitividad \cite{palos-sanchez_modelos_2019}. Históricamente, la infraestructura tecnológica dependía casi por completo de servidores físicos locales (entornos \textit{On-Premise}); sin embargo, la exigencia de agilidad operativa ha impulsado la adopción masiva del \textit{Cloud Computing} \cite{palos-sanchez_modelos_2019}. Este modelo de despliegue dinámico permite aprovisionar redes, servidores y aplicaciones bajo demanda, transformando equipos estáticos en servicios flexibles que escalan sin requerir un alto esfuerzo administrativo \cite{palos-sanchez_modelos_2019}.

A pesar de estas ventajas, la administración de arquitecturas híbridas y distribuidas presenta retos técnicos significativos. Los métodos tradicionales de soporte operan bajo un esquema reactivo, caracterizado por la falta de integración y la dependencia de la gestión manual de incidentes \cite{Arias_Pro_2025}. Esta metodología genera retrasos inaceptables en la identificación de fallas, lo que impacta directamente en la disponibilidad de los servicios \cite{Arias_Pro_2025}. Para mitigar estas interrupciones, la industria exige evolucionar hacia un enfoque centralizado y proactivo, respaldado por marcos de referencia como ITIL, e integrando herramientas de observabilidad que permitan predecir cuellos de botella antes de que afecten la operación y los SLA \cite{saavedra_implementacion_2023}.

Para dar respuesta a esta necesidad latente, el presente documento detalla el desarrollo e implementación del proyecto \textbf{\NombreProyecto}. Este trabajo técnico se ejecuta como parte de las estadías profesionales dentro del Centro de Operación y Excelencia Multicloud (CCoE) de la empresa Honne S.A. de C.V., teniendo como objetivo la optimización y documentación de las operaciones de monitoreo preventivo y soporte de TI para clientes corporativos de alta disponibilidad.

La solución técnica planteada se estructura sobre cuatro ejes operativos principales: la gestión rigurosa del ciclo de vida de los incidentes mediante plataformas ITSM (ServiceNow y ZenDesk) para garantizar el estricto cumplimiento de los Acuerdos de Nivel de Servicio (SLA); la validación sistemática de integraciones \textit{middleware} empleando TIBCO Scribe y Business Works; el monitoreo proactivo de la infraestructura multi-nube a través de consolas nativas y herramientas de telemetría (OpManager, DataDog); y la depuración de paneles de visualización centralizada en Grafana y Prometheus, consolidando métricas para agilizar la toma de decisiones.

\subsection{Antecedentes}
\label{sec:antecedentes}
En los últimos años, la gestión de infraestructuras de Tecnologías de la Información (TI) ha experimentado una transición acelerada desde centros de datos puramente físicos (On-Premise) hacia ecosistemas híbridos y multi-nube. Esta evolución ha permitido a las organizaciones escalar sus operaciones, pero también ha incrementado exponencialmente la complejidad de su administración\cite{palos-sanchez_modelos_2019}.

Históricamente, el soporte técnico operaba bajo un modelo reactivo: las mesas de ayuda (Help Desk) esperaban a que el usuario final reportara una falla para iniciar el proceso de resolución. Sin embargo, en entornos corporativos de alta disponibilidad, este modelo resulta ineficiente y riesgoso, ya que el tiempo de inactividad de un servidor o la interrupción de una interfaz de datos se traduce en pérdidas económicas y operativas significativas\cite{Arias_Pro_2025}.

Para mitigar estos riesgos, la industria ha migrado hacia la implementación de Centros de Operaciones de Red y Seguridad (NOC/SOC), cuyo enfoque principal es el monitoreo proactivo. A pesar de contar con herramientas de monitoreo, muchas organizaciones aún enfrentan el reto de unificar sus alertas, depurar falsos positivos y gestionar eficientemente los Acuerdos de Nivel de Servicio (SLA) en plataformas de Gestión de Servicios de TI (ITSM). El presente proyecto surge ante la necesidad de optimizar estas operaciones de monitoreo y la atención de incidentes para garantizar la continuidad del servicio en infraestructuras complejas\cite{Arias_Pro_2025}.
