...
Para ello, se verificaron alertas activas de AWS mediante Amazon CloudWatch, distinguiendo falsos positivos de incidentes reales. También se revisaron logs de instancias en Google Cloud Platform para detectar errores y problemas de latencia. Además, se dio seguimiento a las soluciones de TIBCO Scribe, inicialmente de forma manual y, más adelante, mediante un script en Python desarrollado por iniciativa propia que automatizó la detección de fallas y su notificación a través de un bot de Telegram. Por último, se dio seguimiento constante a las alertas de ServiceNow. El desarrollo del script requirió aplicar conocimientos de la materia de Redes, además de técnicas de ingeniería inversa de APIs, ya que la plataforma utilizada no contaba con documentación pública para ese fin.
...
This report documents the activities carried out during the University Internship (Estadía) at \UnidadEconomica, a technology transformation and cloud consulting firm, at its Cloud Operations and Multicloud Excellence Center (CCoE) located at calle 533 Maratines, Ciudad Victoria, Tamaulipas. The internship took place in the CCoE's Service Desk area, whose main objective was to support the use of the area's various monitoring tools and its daily activities, helping to distribute the team's workload.

To this end, active AWS alerts were verified using Amazon CloudWatch, distinguishing false positives from real incidents. Logs of instances in Google Cloud Platform were also reviewed to detect errors and latency issues. In addition, TIBCO Scribe solutions were monitored, initially by hand and, later on, through a Python script developed on personal initiative that automated failure detection and notification through a Telegram bot. Lastly, ServiceNow alerts were monitored continuously. Developing the script required applying knowledge from the Networking course, as well as API reverse-engineering techniques, since the platform used did not have public documentation for that purpose.
...
\clearpage
\input{capitulo1_generalidades}

\clearpage
\input{capitulo2_planteamiento}

\clearpage
\input{capitulo3_marcoteorico}
